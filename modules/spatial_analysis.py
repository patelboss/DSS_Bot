...
def load_vector_file(file_path: str | Path) -> tuple[dict, Any]:
    """
    Reads any supported vector format (.kml, .gpkg, .geojson) via GeoPandas.
    Returns:
      1. geojson_feature (dict): The dissolved/unified shape for raster masking.
      2. gdf_attributes (GeoDataFrame): The clean attribute table data frame with explicit fids.
    """
    import fiona
    import geopandas as gpd
    ...
    # 🚀 EXPLICIT FID MAPPING: Assign clear feature IDs if not present
    if "fid" not in gdf.columns:
        gdf.insert(0, "fid", gdf.index + 1)

    # Clean out empty/system noise columns often generated by Google Earth KMLs
    noise_cols = ["Description", "description", "tessellate", "extrude", "visibility"]
    for col in noise_cols:
        if col in gdf.columns and col != gdf.columns:  # don't drop if it's the only column
            gdf = gdf.drop(columns=[col], errors="ignore")

    # Generate the single dissolved feature geometry for your Supabase COG analysis pipeline
    if gdf.crs is None:
    ...
    # Return both the geometry dictionary and the un-dissolved data frame for table extraction
